...
Note that if an event arises more than once within the tick interval, it is automatically
...
.. note::
    This type of item is only available when the ``dbus`` feature is enabled; however, both the
    standard Linux configuration and the provided binaries come with the feature enabled.
...
.. note::
    This type of item is only available when the ``wmi`` feature is enabled; however, both the
    standard Windows configuration and the provided binaries come with the feature enabled.
...
This type of event is directly handled by the main program, and is not caught by the event
listener.

.. tip::
    This type of event might seem of little or no use. In fact, for instance, `When`_ itself does
    not implement or exploit it. However, in case a frontend is made available for a particular
    platform, that is able to react to signals that are not directly handled by **whenever**,
    it can trigger specific events directly in order to exploit **whenever** as a context where
