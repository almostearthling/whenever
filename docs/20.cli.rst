...
* if a launcher behaves differently according to optional features available in the **whenever**
  ...
  - ``lua_sync``: the embedded *Lua* interpreter supports synchronization and data sharing
    among scripts that run at different times
  - ``lua_httpreq``: the embedded *Lua* interpreter is capable of performing HTTP request
    without external tools
  - ``lua_unsafe``: the embedded *Lua* interpreter allows to call external functions: this
    feature is experimental and unsupported, and usually disabled in binary releases.
