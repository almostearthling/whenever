...
* if a launcher behaves differently according to optional features available in the **whenever**
  executable, it can use the ``--options`` switch to determine which ones are available, the
  command output is the string ``options:`` followed by one or more of the following:
  - ``dbus``: the executable has been compiled with DBus support (generally on Linux)
  - ``wmi``: the executable has WMI support (available on Windows only)
  - ``lua_sync``: the embedded *Lua* interpreter supports synchronization and data sharing among scripts that run at different times
  - ``lua_httpreq``: the embedded *Lua* interpreter is capable of performing HTTP request without external tools
  - ``lua_unsafe``: the embedded *Lua* interpreter allows to call external functions: this feature is experimental and unsupported, and usually disabled in binary releases.

For the *Lua* related features, please refer to the specific documentation pages.
...
.. [#fn-1] Executables compiled in *debug mode*, however, do not consider an active release
           ...
           instance to a debug instance, and an active release instance to a release instance.
