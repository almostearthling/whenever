...
have been released with version **1.88.0** have been used. Once the *Rust* requirement, along with
...
.. code-block:: shell
   ...
   cargo build --release --features linux_std

2. on Windows:

.. code-block:: shell

   cargo build --release --features windows_std
...
default. Taking Debian as an example, the following packages should be installed:

* *pkg-config*
* *libx11-dev*
* *libxss-dev*.

In fact, *X11* libraries are optional, and all features are now supported on Wayland desktops as
well; however, in particular, *idle time detection* can fail on Wayland desktops if *X11* support
is not available.

To enable the `lua_httpreq` feature, the *libssl-dev* package must be installed on Linux systems.
