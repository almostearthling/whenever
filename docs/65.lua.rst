...
and the answer that *httpbin.org* provides is the following:
...
.. warning::
  ...
  *string*: any other type of value, including *nil*, will cause an error at the end of the
  script: in this case the state is *not* saved.
...
*Shared states* are accessible to *all* lua scripts, and behave differently: they need to be
explicitly loaded and saved by a script, respectively using the ``sharedstate.load(name)`` and
``sharedstate.save(name, table)`` functions.
...
``timeout`` parameter can also be a fractional number.
...
.. [#fn-2] The ``package.cpath`` setting can be modified as well, however it generally has no
