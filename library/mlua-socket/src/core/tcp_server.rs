...
impl UserData for TcpServer {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_method("close", |lua, tcp_server, ()| {
            super::tcp_close::handle(lua, &tcp_server.tcp)
        });
        methods.add_method("getfd", |lua, tcp_server, args| {
            match super::tcp_getfd::handle(lua, &tcp_server.tcp, args) {
                Ok(fd) => {
        ...
        methods.add_method(
            "getsockname",
            |lua, tcp_server, args| match super::tcp_getsockname::handle(lua, &tcp_server.tcp, args) {
                Ok((addr, port, family)) => {
                    let mut retval = MultiValue::new();
        ...
        methods.add_method("listen", |lua, tcp_server, args| {
            match super::tcp_listen::handle(lua, &tcp_server.tcp, args) {
                Ok(()) => {
                    let mut retval = MultiValue::new();
                    retval.push_front(Value::Number(1.));
                    Ok(retval)
        ...
        methods.add_method("setoption", |lua, tcp_server, args| {
            match super::tcp_setoption::handle(lua, &tcp_server.tcp, args) {
                Ok(_) => {
                    let mut retval = MultiValue::new();
                    retval.push_front(Value::Number(1.));
                    Ok(retval)
        ...
        methods.add_method("settimeout", |lua, tcp_server, args| {
            match super::tcp_settimeout::handle(lua, &tcp_server.tcp, args) {
                Ok(_) => {}
                Err(_e) => {}
            }
            Ok(mlua::Nil)
