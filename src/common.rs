...
///
/// This module implements a global set of named mutexes that can be locked and
/// released by any thread.
pub mod named_mutex {
    // *** IMPORTANT NOTE: this module has been almost generated by AI ***
    // Generated by: Copilot/Claude Haiku 4.5, fixed
    //
    // 2026/03/12 - This is the first time that I introduce any AI generated
    // code in this utility; it is a simple structure compare to other ones
    // found throughout the source base and I surely would have been able to
    // implement it by myself. But it is simple enough and complex enough at
    // the same time, for me to see how AI assistant behave when asked to
    // produce code based on a prompt. However, I had to fix it: Claude gave
    // for granted that the standard std::sync::Mutex provides a timeout
    // fenced `try_lock_for()` function, which is not true. I had to switch
    // to the Mutex provided in the parking_lot crate (it is quite popular,
    ...
    pub fn namedmutex_lock(name: &str, timeout: Option<Duration>) -> bool {
        ...
        let mutex_arc = {
            let mut mutex_map = get_mutex_map().lock();
            mutex_map
        ...
        let result = if let Some(duration) = timeout {
            ...
            if mutex_arc.try_lock_for(duration).is_some() {
