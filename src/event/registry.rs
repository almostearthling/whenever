...
impl EventRegistry {
    ...
    /// May panic if the event registry could not be locked for enquiry
    /// or the contained event cannot be locked for comparison.
    fn service_running_for(&self, name: &str) -> bool {
...
impl StelEventRegistry {
    ...
    pub fn run_event_listener(&'static self) -> Result<()> {
        ...
        async fn next_event(registry: Arc<Mutex<&StelEventRegistry>>) -> TriggeredOrQuitMessage {
            let r0 = registry.clone();
            ...
            // generally because of a reconfiguration
            let mut el0 = el0.lock().expect("cannot acquire event list");
            let catch_events = el0.iter_mut().map(|(_, evt)| evt.stel_event_triggered());
    ...
    // /// Check whether the specified event is among the listened-for ones
    // ///
    // /// # Arguments
    // ///
    // /// * name - the event name
    ...
    // /// May panic if the event registry could not be locked for enquiry
    // /// or the contained event cannot be locked for comparison.
    // fn is_listening_for(&self, name: &str) -> bool {
    //     self.has_event(name)
    // }
    ...
    //     let id = self.event_id(name).unwrap();
    //     let m0 = self.listener_quit_messenger.clone();
    //     let mut m1 = m0.lock().unwrap();
    //     if m1.is_some() {
    //         let messenger = m1.as_mut().unwrap();
    //         futures::executor::block_on(async move {
