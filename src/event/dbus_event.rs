...
impl Event for DbusMessageEvent {
    ...
    fn run_service(&self, qrx: Option<mpsc::Receiver<()>>) -> std::io::Result<bool> {
        ...
        enum TargetOrQuitEvent {
            Target(Result<Option<Arc<Message>>, zbus::Error>),
            Quit,
            QuitError,
        }
        ...
        async fn _pass_messages(stream: &mut MessageStream, mut tx: Sender<TargetOrQuitEvent>) {
            let m = stream.try_next().await;
            tx.send(TargetOrQuitEvent::Target(m)).await.unwrap();
        }
        ...
        let mut dbus_stream = dbus_stream.unwrap();
        ...
        futures::executor::block_on(async { 'outer: loop {

            // this internally uses `try_next` so it actually shouldn't block
            _pass_messages(&mut dbus_stream, async_tx.clone()).await;
